...
def predict_from_set(model_folder):
    ...
    print(np.divide(H_matrix, np.multiply(ss_count_matrix[0], aa_freq_matrix)))
    print(np.divide(E_matrix, np.multiply(ss_count_matrix[1], aa_freq_matrix)))
    print(np.divide(C_matrix, np.multiply(ss_count_matrix[2], aa_freq_matrix)))
    
    for line in open_idlist:
        profile_name = line.rstrip() + ".profile"
        profile_path = os.path.join(profile_folder, profile_name)
        predict_ss(profile_path, inf_matrix_H, inf_matrix_E, inf_matrix_C, ss_count_matrix, aa_freq_matrix)
   
    return(inf_matrix_H, inf_matrix_E, inf_matrix_C)



def predict_ss(profile_file, inf_matrix_H, inf_matrix_E, inf_matrix_C, ss_count_matrix, aa_freq_matrix):
    ...
    for line in profile_opened:
        ...
        '''p_RS = H_matrix
        p_R = aa_freq_matrix
        p_S = ss_count_matrix[0]
        p_SR_H = np.log(p_RS/(p_R*p_S))
        print(p_SR_H)'''
